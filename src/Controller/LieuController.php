...
class LieuController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $entityManager->flush();
            $this->addFlash('bg-success text-white', 'Le lieu a bien été créé.');
    ...
    public function edit(Request $request, Lieu $lieu, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->flush();
            $this->addFlash('bg-warning text-dark', 'Le lieu a bien été modifé');
    ...
    public function delete(Request $request, Lieu $lieu, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('delete'.$lieu->getId(), $request->request->get('_token'))) {
            ...
            $entityManager->flush();
            $this->addFlash('bg-danger text-white', 'Le lieu a bien été supprimé');
        }
